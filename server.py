...
class ResearchStreamRequestModel(BaseModel):
    topic: str
...
def get_authenticated_user(authorization: str = Header(None)):
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail=f"Invalid token: {str(validation_error)}")
...
def get_optional_user(authorization: str = Header(None)):
    ...
    try:
        ...
        decoded_token_data = jwt.decode(bearer_token, JWT_SECRET, algorithms=["HS256"])
        if db is not None:
            return db.users.find_one({"_id": ObjectId(decoded_token_data["user_id"])})
        return {"_id": ObjectId(decoded_token_data["user_id"]), "email": "mock@example.com"}
    except Exception:
        return None

@app.get("/")
def index():
    return {"status": "online", "message": "ARCS FastAPI Backend API is running successfully."}
...
def get_stages():
    ...
    return {"stages": formatted_stages}

@app.post("/api/auth/register", status_code=201)
def register(payload: UserRegistrationModel):
    try:
...
def login(payload: UserLoginModel):
    try:
        normalized_email = payload.email.strip().lower()
        user_password = payload.password
        ...
        if db is not None:
            ...
            if normalized_email == "mock@example.com" and user_password == "password":
                ...
                raise HTTPException(status_code=401, detail="Invalid credentials (use mock@example.com / password)")
...
@app.get("/api/history")
def get_history(authenticated_user: dict = Depends(get_authenticated_user)):
    try:
        history_dossiers = []
        if db is not None:
        ...
        return {'history': history_dossiers, 'status': 'success'}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))

@app.get("/api/history/{record_id}")
def get_history_detail(record_id: str, authenticated_user: dict = Depends(get_authenticated_user)):
    try:
        if db is not None:
...
def research_stream(payload: ResearchStreamRequestModel, optional_user: dict = Depends(get_optional_user)):
    target_topic = payload.topic.strip()
    if not target_topic:
    ...
    def stream_research_pipeline_execution():
        ...
        try:
            ...
            if optional_user and db is not None:
                try:
                    db.history.insert_one({
                        "user_id": optional_user["_id"],
                        "topic": target_topic,
    ...
    return StreamingResponse(
        stream_research_pipeline_execution(),
...
if __name__ == '__main__':
    ...
    print(f"📡 Starting ARCS FastAPI backend server on port {listening_port}")
